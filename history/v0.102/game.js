(function () {
            ...
            function drawScoreboard() {
                ctx.fillStyle = "black";
                ctx.font = "20px Arial";
                ctx.fillText("Player 1: " + state.left.score, 10, 30);
                ctx.fillText("Player 2: " + state.right.score, canvas.width - 150, 30);
            }

            function updateAI() {
                const aiPaddle = state.right;
                const ballY = state.ball.y;
                const paddleHeight = paddleHeight();
                const paddleSpeed = powerUpActive ? 7 : 5;

                if (ballY < aiPaddle.y + paddleHeight / 2) {
                    aiPaddle.y -= paddleSpeed;
                } else if (ballY > aiPaddle.y + paddleHeight / 2) {
                    aiPaddle.y += paddleSpeed;
                }
                ...
                if (aiPaddle.y < 0) {
                    aiPaddle.y = 0;
                } else if (aiPaddle.y > canvas.height - paddleHeight) {
