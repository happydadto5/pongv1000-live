(function () {
    ...
    function gameLoop() {
        update();
        drawNet();
        drawPaddle({ x: state.left.x, y: state.left.y, width: paddleWidth(), height: paddleHeight() });
        drawPaddle({ x: state.right.x, y: state.right.y, width: paddleWidth(), height: paddleHeight() });
        drawBall();
